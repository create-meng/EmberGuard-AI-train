...
class FileHandler:
    ...
    @staticmethod
    def save_detection_results(detection_results, detection_file_type, selected_file_path, 
                               add_info_callback, show_message_callback):
        """保存检测结果"""
        # 检查是否有检测结果
        ...
        try:
            ...
            add_info_callback(f"保存检测结果时发生错误: {str(e)}")
            messagebox.showerror("错误", f"保存检测结果时发生错误: {str(e)}")
            return False
