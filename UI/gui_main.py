...
try:
    # 尝试相对导入（当作为包的一部分运行时）
    from .config import DND_AVAILABLE, DND_FILES, MODEL_PATH, WINDOW_SIZE, WINDOW_TITLE
    ...
    from hys.config import MODEL_PATH, WINDOW_SIZE, WINDOW_TITLE
...
class YOLODetectionGUI:
    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            self.model_loaded = False
            messagebox.showerror("错误", f"无法加载YOLO模型: {e!s}")
            return False
    ...
    def select_model(self):
        ...
        model_path = filedialog.askopenfilename(
            title="选择YOLO模型文件", filetypes=[("PyTorch模型", "*.pt"), ("所有文件", "*.*")], initialdir="."
        )
    ...
    def show_mode_selection(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="YOLO 目标检测系统", font=("Arial", 20, "bold"))
        title_label.pack(pady=20)

        # 副标题
        subtitle_label = ttk.Label(main_frame, text="请选择检测模式", font=("Arial", 12))
        ...
        # 选择模型按钮
        select_model_button = ttk.Button(model_frame, text="📦 选择模型", command=self.select_model, width=20)
        ...
        status_hint = ttk.Label(model_frame, text=status_text, font=("Arial", 9), foreground=status_color)
        ...
        save_dir_label = ttk.Label(save_info_frame, text=save_dir_display, font=("Arial", 9), foreground="blue")
        ...
        # 选择保存文件夹按钮
        select_save_button = ttk.Button(save_frame, text="📁 选择保存文件夹", command=self.select_save_folder, width=20)
        select_save_button.pack(pady=5)

        # 保存文件夹说明
        save_hint = ttk.Label(
            save_frame, text="检测到目标时，会自动保存帧到此文件夹", font=("Arial", 8), foreground="gray"
        )
        ...
        screen_button = ttk.Button(
            button_frame, text="🖥️ 屏幕检测", command=lambda: self.select_mode("screen"), width=25
        )
        screen_button.pack(pady=15, padx=10)

        # 摄像头检测按钮
        camera_button = ttk.Button(
            button_frame, text="📷 摄像头检测", command=lambda: self.select_mode("camera"), width=25
        )
        camera_button.pack(pady=15, padx=10)

        # 文件检测按钮
        file_button = ttk.Button(
            button_frame, text="📁 文件检测（图片/视频）", command=lambda: self.select_mode("file"), width=25
        )
    ...
    def create_detection_ui(self, mode_name):
        """创建检测界面."""
        ui_dict = DetectionUI.create_detection_ui(self.root, mode_name, self.show_mode_selection)
        ...
        # 保存按钮引用
        self.buttons = {"start": ui_dict["start_button"], "stop": ui_dict["stop_button"]}
    ...
    def _validate_and_get_params(self):
        ...
        try:
            ...
            messagebox.showerror("参数错误", f"读取参数时发生错误: {e!s}")
            return None, None
    ...
    def save_detection_results(self):
        ...
        FileHandler.save_detection_results(
            self.detection_results, self.detection_file_type, self.selected_file_path, add_info, show_message
        )
    ...
    def select_save_folder(self):
        ...
        folder = filedialog.askdirectory(
            title="选择保存文件夹", initialdir=self.save_dir if os.path.exists(self.save_dir) else "."
        )
