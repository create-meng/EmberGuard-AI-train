...
try:
    # 尝试相对导入（当作为包的一部分运行时）
    from .config import DEFAULT_CONF, DEFAULT_IOU, DND_AVAILABLE, DND_FILES
except ImportError:
    # 如果相对导入失败，使用绝对导入（直接运行时）
    # 添加父目录到路径
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    try:
        from hys.config import DEFAULT_CONF, DEFAULT_IOU, DND_AVAILABLE, DND_FILES
    except ImportError:
        # 如果都失败，使用默认值
        DND_AVAILABLE = False
        DND_FILES = None
...
class DetectionUI:
    ...
    def create_detection_ui(root, mode_name, control_frame_callback=None):
        ...
        title_label = ttk.Label(header_frame, text=f"YOLO {mode_name}", font=("Arial", 16, "bold"))
        title_label.pack(side=tk.LEFT)

        # 返回按钮
        back_button = ttk.Button(header_frame, text="← 返回模式选择", command=control_frame_callback)
        ...
        video_label = ttk.Label(
            video_frame, text="准备中...", background="black", foreground="white", font=("Arial", 12), anchor="center"
        )
        ...
        help_label = ttk.Label(params_frame, text=help_text, font=("Arial", 8), foreground="gray")
        ...
        start_button = ttk.Button(button_frame, text="开始检测", command=lambda: None)  # 将在外部设置
        start_button.pack(side=tk.LEFT, padx=5)

        stop_button = ttk.Button(button_frame, text="停止检测", state=tk.DISABLED)
        stop_button.pack(side=tk.LEFT, padx=5)

        # 状态标签
        status_label = ttk.Label(control_frame, text="状态: 未启动", font=("Arial", 10))
    ...
    @staticmethod
    def setup_file_detection_ui(
        control_frame, video_label, select_file_callback, on_file_drop_callback, save_results_callback
    ):
        """设置文件检测界面的额外组件."""
        # 添加文件选择区域
        ...
        select_button = ttk.Button(file_select_frame, text="选择文件", command=select_file_callback)
        ...
        # 提示文字
        hint_label = ttk.Label(file_select_frame, text="或拖拽文件到显示区域", font=("Arial", 9), foreground="gray")
        hint_label.pack(side=tk.LEFT, padx=10)

        # 保存结果按钮
        save_button = ttk.Button(control_frame, text="保存检测结果", command=save_results_callback, state=tk.DISABLED)
