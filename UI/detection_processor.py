...
class DetectionProcessor:
        ...
        try:
            ...
            if hasattr(result, "orig_img"):
                ...
                if self.save_frame_count % 10 == 0:
                    self.gui_updater.add_info(
                        self.info_text, f"已保存 {self.save_frame_count} 帧到: {os.path.basename(self.save_dir)}"
                    )
        except Exception:
            # 保存失败时不中断检测，只记录错误
            pass
    ...
    def start_screen_detection(self):
        ...
        try:
            ...
            self.gui_updater.update_status(self.status_label, f"错误: {e!s}")
            self.gui_updater.add_info(self.info_text, f"启动屏幕检测时发生错误: {e!s}")
            self.is_running = False
    ...
    def start_camera_detection(self):
        ...
        try:
            ...
            self.gui_updater.update_status(self.status_label, f"错误: {e!s}")
            self.gui_updater.add_info(self.info_text, f"启动摄像头时发生错误: {e!s}")
            self.is_running = False

    def start_file_detection(
        self, file_path, detection_file_type_callback, detection_results_callback, detection_has_results_callback
    ):
        """开始文件检测."""
        if self.is_running:
            return
        ...
        try:
            ...
            self.gui_updater.update_status(self.status_label, f"错误: {e!s}")
            self.gui_updater.add_info(self.info_text, f"处理文件时发生错误: {e!s}")
            from tkinter import messagebox

            messagebox.showerror("错误", f"处理文件时发生错误: {e!s}")
            self.is_running = False
            self.gui_updater.update_button_state(self.buttons, "start", tk.NORMAL)
            self.gui_updater.update_button_state(self.buttons, "stop", tk.DISABLED)
            if "save" in self.buttons:
    ...
    def process_screen(self):
        ...
        try:
            ...
            for result in self.yolo(source="screen", stream=True, verbose=False, conf=self.conf, iou=self.iou):
            ...
            self.gui_updater.add_info(self.info_text, f"屏幕检测时发生错误: {e!s}")
            self.is_running = False
            self.gui_updater.update_button_state(self.buttons, "start", tk.NORMAL)
            self.gui_updater.update_button_state(self.buttons, "stop", tk.DISABLED)
    ...
    def process_camera(self):
        ...
        try:
            # source=0 表示摄像头
            for result in self.yolo(source=0, stream=True, verbose=False, conf=self.conf, iou=self.iou):
                if not self.is_running:
                    break
            ...
            self.gui_updater.add_info(self.info_text, f"摄像头检测时发生错误: {e!s}")
        finally:
    ...
    def process_video_file(self, file_path, detection_results_callback, detection_has_results_callback):
        ...
        try:
            ...
            for result in self.yolo(source=file_path, stream=True, verbose=False, conf=self.conf, iou=self.iou):
                if not self.is_running:
                    break
            ...
            self.gui_updater.add_info(self.info_text, f"处理视频时发生错误: {e!s}")
            self.is_running = False
            self.gui_updater.update_button_state(self.buttons, "start", tk.NORMAL)
            self.gui_updater.update_button_state(self.buttons, "stop", tk.DISABLED)
            if "save" in self.buttons:
